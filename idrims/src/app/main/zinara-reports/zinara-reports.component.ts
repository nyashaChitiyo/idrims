...
@Component({
  selector: 'app-zinara-reports',
  templateUrl: './zinara-reports.component.html',
  styleUrls: ['./zinara-reports.component.css']
})
export class ZinaraReportsComponent implements OnInit {
  ...
  getAll(){
    this.demo.get('http://108.61.174.41:7070/api/reports/view/zinara/all')
    .subscribe(
      (data:any[])=> { 
        let arr = [];
        arr.push(data)
  ...
  getByCollectionPoint(){
    ...
    this.demo.post('http://108.61.174.41:7070/api/reports/view/zinara/collectionPoint',{
      "id": +this.selectedReg
    })
    .subscribe(
      (data: Response)=> {
  ...
  getByUserID(){
    this.demo.post('http://108.61.174.41:7070/api/reports/view/zinara/userId',{
      "id": this.userid
    })
    .subscribe(
      (data: Response)=> {

...
@Component({
  selector: 'app-zbc-reports',
  templateUrl: './zbc-reports.component.html',
  styleUrls: ['./zbc-reports.component.css']
})
export class ZbcReportsComponent implements OnInit {
  ...
  getAll(){
    this.demo.get('http://108.61.174.41:7070/api/reports/view/zbc/all')
    .subscribe(
      (data:any[])=> { 
        let arr = [];
        arr.push(data)
  ...
  getByCollectionPoint(){
    ...
    this.demo.post('http://108.61.174.41:7070/api/reports/view/zbc/collectionPoint',{
      "id": +this.selectedReg
    })
    .subscribe(
      (data: Response)=> {
  ...
  getByUserID(){
    this.demo.post('http://108.61.174.41:7070/api/reports/view/zbc/userId',{
      "id": this.userid
    })
    .subscribe(
      (data: Response)=> {
